    ...
    filtered.forEach(task => {
        ...
        element.innerHTML = `
            ...
            <div>
                ...
                ${
                    task.created_by
                        ? `<div class="notes">Added by ${escapeHTML(task.created_by)}</div>`
                        : ""
                }
                ${
                    task.notes
...
function getDisplayName(user) {
    return user?.user_metadata?.display_name || user?.email || "Someone";
}
...
async function startApp(user) {
    currentUser = user;
    document.getElementById("welcomeName").textContent = `Signed in as ${getDisplayName(user)}`;
...
document.getElementById("signUpBtn").addEventListener("click", async () => {
    ...
    if (!name) {
        authError.textContent = "Enter a display name so friends know it's you.";
        return;
    }

    if (!email || !password) {
        authError.textContent = "Enter an email and password too.";
        return;
    }
